 ...
 *  NOTE: entitySecretCiphertext is deliberately NOT a header — see file
 *  header. It belongs in the body of each individual mutating request,
 *  freshly generated every time; see getEntitySecretCiphertext(). */
...
// ── Entity secret ciphertext (real implementation) ─────────────────────────
...
async function getEntitySecretCiphertext(): Promise<string> {
  ...
  if (entitySecretBuffer.length !== 32) {
    throw new Error(
      `CIRCLE_ENTITY_SECRET must be a 32-byte hex-encoded string (64 hex chars) — got ${entitySecretBuffer.length} bytes. Generate one with Circle's entity-secret sample code, not an arbitrary string.`,
    );
  }

  const publicKeyPem = await fetchEntityPublicKey();
...
// UUID, not an arbitrary string. This codebase's design intentionally
// derives a STABLE key per employer wallet (see agentIdentity.ts) so a
// retried/duplicate activate() call returns the same Circle wallet
// instead of creating a second one — that idempotency behavior still
// needs to work, it just needs to produce valid UUID syntax. UUID v5
// (RFC 4122 §4.3, SHA-1 based) turns an arbitrary string into a stable,
// deterministic UUID: same input always produces the same UUID, with no
// external state or database needed.
const SALDEN_UUID_NAMESPACE = '6ba7b810-9dad-11d1-80b4-00c04fd430c8'; // DNS namespace UUID — used only as a fixed seed, not semantically meaningful here

function toUUIDv5(name: string): string {
